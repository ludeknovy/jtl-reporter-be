...
import { uploadExecutionFilesController }
    from "../controllers/scenario/execution-files/upload-execution-files-controller"
import {deleteExecutionFileController} from "../controllers/scenario/execution-files/delete-execution-file-controller";
import {getExecutionFileController} from "../controllers/scenario/execution-files/get-execution-files-controller";
...
export class ScenarioRoutes {
    ...
    routes(app: express.Application): void {
        ...
        app.route("/api/projects/:projectName/scenarios/:scenarioName/execution-files")
            .post(
                authenticationMiddleware,
                authorizationMiddleware([AllowedRoles.Operator, AllowedRoles.Admin]),
                paramsSchemaValidator(paramsSchema),
                uploadExecutionFilesController)

            .get(authenticationMiddleware,
                authorizationMiddleware([AllowedRoles.Operator, AllowedRoles.Admin]),
                paramsSchemaValidator(paramsSchema),
                wrapAsync((req: Request, res: Response) => getExecutionFileController(req, res))
                )


        app.route("/api/projects/:projectName/scenarios/:scenarioName/execution-files/:fileId")
            .delete(
                authenticationMiddleware,
                authorizationMiddleware([AllowedRoles.Operator, AllowedRoles.Admin]),
                paramsSchemaValidator(paramSchemaExecutionFile),
                wrapAsync((req: Request, res: Response) => deleteExecutionFileController(req, res)))

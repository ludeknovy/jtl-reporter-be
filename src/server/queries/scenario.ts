...
export const searchScenarioShareTokens = (projectName: string, scenarioName: string) => {
    return {
        text: `SELECT t.id, t.token, t.note FROM jtl.scenario_share_tokens as t
    LEFT JOIN jtl.scenario as s ON s.id = t.scenario_id
    LEFT JOIN jtl.projects as p ON p.id = s.project_id
    WHERE p.project_name = $1
    AND s.name = $2;`,
...
export const selectOnlyMyScenarioShareTokens = (projectName: string, scenarioName: string, userId: string) => {
    return {
        text: `SELECT t.id, t.token, t.note FROM jtl.scenario_share_tokens as t
    LEFT JOIN jtl.scenario as s ON s.id = t.scenario_id
    LEFT JOIN jtl.projects as p ON p.id = s.project_id
